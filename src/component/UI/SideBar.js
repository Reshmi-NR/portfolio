...
const SideBar = (props) => {
    ...
    return (
        <div className="flex ">
            ...
            <div className={`${open ? "w-72 " : "w-11"} duration-300 h-screen bg-dark-purple text-3xl absolute left-0 top-0`}>      
    ...
    >
      ...
      <path d="M5 8a.5.5 0 01.5-.5h7a.5.5 0 010 1h-7A.5.5 0 015 8zm0-2.5a.5.5 0 01.5-.5h7a.5.5 0 010 1h-7a.5.5 0 01-.5-.5zm0 5a.5.5 0 01.5-.5h7a.5.5 0 010 1h-7a.5.5 0 01-.5-.5zm-1-5a.5.5 0 11-1 0 .5.5 0 011 0zM4 8a.5.5 0 11-1 0 .5.5 0 011 0zm0 2.5a.5.5 0 11-1 0 .5.5 0 011 0z" />
                ...
                <div>
                  <ul className="pt-6 divide-y divide-blue-200 divide-solid">
                    {props.menus.map((menus,index)=>(
                      <li key={index} className={`text-gray-500 text-sm flex items-center gap-x-4 cursor-pointer p-2 hover:bg-light-white rounded-md ${menus.gap ? "mt-9" : "mt-2"}`}>
                       <img src={menus.src} />
                      <span className={`${!open ?  "invisible" : "visible"} origin-left duration-200`}>{menus.title}</span>
                        </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className= {`px-7 text-2xl font-semibold text-gray-500 ${!open ? "absolute top-2 left-6" : "absolute top-2 left-1/4"}`} >
            <h1>
